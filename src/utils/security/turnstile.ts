...
export function loadTurnstile(): Promise<void> {
  ...
  if (scriptLoaded) {
    return new Promise((resolve) => {
      const wait = () => (window.turnstile ? resolve() : window.setTimeout(wait, 50))
      wait()
    })
  }

  if (!SITE_KEY) return Promise.reject(new Error('Missing VITE_TURNSTILE_SITE_KEY'))
...
function normalizeTarget(target: string | HTMLElement): string | HTMLElement {
  return typeof target === 'string' && !target.startsWith('#') ? '#' + target : target
}
...
export async function renderTurnstile(
  ...
  const target = normalizeTarget(container)
  widgetId = window.turnstile.render(target as any, {
    sitekey: SITE_KEY,
