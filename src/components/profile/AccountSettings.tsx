...
export default function AccountSettings() {
  ...
  const [emailChangeBannerTo, setEmailChangeBannerTo] = useState<string | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      fetch(
        `${API_BASE}/auth/check-username?username=${encodeURIComponent(
          trimmed
        )}`
      )
        .then((r) => r.json())
  ...
  const saveUsername = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        showToast(
          data?.error || "We couldn’t update your username. Please try again.",
          "error"
        );
      }
    } catch {
  ...
  const saveEmail = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        showToast(
          "Verification sent! Check your new inbox to confirm.",
          "success"
        );
      } else {
        showToast(
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      showToast(
        msg || "We couldn’t delete your account. Please try again.",
        "error"
      );
    } catch {
      showToast(
          ...
          {emailChangeBannerTo && (
            <div className="mx-6 mt-4 rounded-md border border-amber-200 bg-amber-50 p-3 flex items-start gap-2">
              ...
              <div className="text-sm text-amber-800">
                ...
                <span className="font-semibold">{emailChangeBannerTo}</span>.
          ...
          <CardContent className="p-6 space-y-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-gray-500">
                This is your display name, derived from your username.
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              {emailChangePending && (
                <p className="text-xs text-amber-600 mt-1">
                  Pending email change — check your new inbox for a confirmation
                  link.
                </p>
              )}
            </div>
            ...
            {me?.canChangePassword ? (
              <div className="space-y-2">
                ...
                <PasswordSection
                  ...
                  onSubmit={async (oldP, newP, confirmP) => {
                    if (newP !== confirmP) {
                      showToast(
                        "The new passwords don’t match. Please re-enter them.",
                        "error"
                      );
                      return;
                    }
              ...
              <div className="rounded-md border p-4 bg-gray-50">
                <p className="text-sm text-gray-600">
                  Your account uses Google sign-in. There’s no local password to
                  change.
                </p>
              </div>
            )}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-red-700/90 text-sm mb-3">
                        This action can’t be undone. Your profile and related
